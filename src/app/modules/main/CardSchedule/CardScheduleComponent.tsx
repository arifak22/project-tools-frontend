...
const ModalForm = ({showModal, setShowModal, refreshData, data}:any) => {
    ...
    const cardSchema = Yup.object().shape({
        ...
        end_date: Yup.date()
          .when("start_date",
            (start_date, yup) => start_date && yup.min(start_date, "End Date cannot be before Start Date")),
        users: Yup.array(),
    ...
    var initialValues = {
        card_event_id: data ? data['card_event_id']: 0,
        id_crypt     : param.id,
        title        : data ? data['title']        : '',
        message      : data ? data['message']      : '',
        start_date   : data ? data['start_date']   : moment().add(3, 'days'),
        end_date     : data ? data['end_date']     : moment().add(3, 'days'),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, {setStatus}) => {
            try {
                ...
                await axios.post(API().postScheduleCreate, values).then((response)=>{
    ...
    const getMember = () => {
        setUsers([]);
        axios.get(API().getUserSchedule + '?id_crypt=' + param.id)
        .then((response) => {
            ...
            >
                ...
                <div className="modal-body">
                    <div className='scroll-y me-n5 pe-5' style={{height:`${height-120}px`}}>
                        ...
                        <TextForm formik={formik} label='Note' name='message' rows={8}/>
                        <div className="row">
                          <div className="col-md-6">
                            <DateTimeForm formik={formik} label='Start Date' name='start_date' />
                          </div>
                          <div className="col-md-6">
                            <DateTimeForm formik={formik} label='End Date' name='end_date' />
                          </div>
                        </div>
                        ...
                        <ButtonUploadFile data={formik.values.lampiran} temp='Y' modul='schedule' onChange={(file:any)=> {
